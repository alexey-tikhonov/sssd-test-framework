...
class IDUserOverride(IPAUser):
    ...
    def add_override(
        ...
        **kwargs,
        ...
        :retun: self, str
        :rtype: IDUserOverride, str
        """
        certs = [certificate] if isinstance(certificate, str) else certificate or []

        valid_keys = {"desc", "login", "uid", "gidnumber", "gecos", "homedir", "shell", "sshpubkey", "certificate"}
        ...
        attrs: CLIBuilderArgs = {
            ...
            "gidnumber": (self.cli.option.VALUE, gid),
            "gecos": (self.cli.option.VALUE, gecos),
            "homedir": (self.cli.option.VALUE, home_directory),
            "shell": (self.cli.option.VALUE, shell),
            "sshpubkey": (self.cli.option.VALUE, sshpubkey),
        }

        # Get string of invalid options for testing negative test cases
        # options like email, principle, first_name, last_name etc.
        invalid_keys = ", ".join([key for key in kwargs if key not in valid_keys])

        for key, value in kwargs.items():
            if key in valid_keys:
                attrs[key] = (self.cli.option.VALUE, value)
    ...
    def find_override(self, idview_name: str) -> list[str]:
        """
        Search for an User ID override.

        :param idview_name: Name of IDView.
        :type idview_name: str
        :return: list of IDOverride information.
        :rtype: list[str]
        """
        result = self.role.host.conn.exec(["ipa", "idoverrideuser-find", idview_name, "--anchor", self.name])
        cleaned_list = [line for line in result.stdout.splitlines() if "-" not in line]
        return cleaned_list

    def show_override(self, idview_name: str) -> list[str]:
        """
        ...
        :rtype: list[str]
        """
        result = self.role.host.conn.exec(["ipa", "idoverrideuser-show", idview_name, self.name, "--all"])
        cleaned_list = [item.strip() for item in result.stdout.splitlines()]
        return cleaned_list
...
class IDGroupOverride(IPAGroup):
    ...
    def add_override(
        ...
        *,
        ...
        :param description: Description of the IDView.
        :type description: str | None, defaults to None.
        :param group_name: Name of the IDView.
        :type group_name: str | None, defaults to None.
        ...
        attrs: CLIBuilderArgs = {
            "desc": (self.cli.option.VALUE, description),
            "group-name": (self.cli.option.VALUE, group_name),
            "gid": (self.cli.option.VALUE, gid),
        }
    ...
    def modify_override(
        ...
        *,
        ...
        :param description: Description of the IDView.
        :type description: str | None, defaults to None
        :param group_name: group name of the IDView.
        :type group_name: str | None, defaults to None
        :param gid: GID of the IDView.
        :type gid: str | None, defaults to None
        :return: self.
    ...
    def find_override(self, idview_name: str) -> list[str]:
        ...
        :param idview_name: Name of IDView.
        :type: idview_name: str
        :return: List of ID override.
        :rtype: list[str]
        """
        find_result = self.role.host.conn.exec(["ipa", "idoverridegroup-find", idview_name, "--anchor", self.name])
        cleaned_list = [line for line in find_result.stdout.splitlines() if "-" not in line]
        return cleaned_list
    ...
    def show_override(self, idview_name: str) -> list[str]:
        ...
        :rtype: list[str]
        """
        show_result = self.role.host.conn.exec(["ipa", "idoverridegroup-show", idview_name, self.name, "--all"])
        cleaned_list = [item.strip() for item in show_result.stdout.splitlines()]
        return cleaned_list
